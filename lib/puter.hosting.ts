...
import {
  createHostingSlug, dataUrlToBlob,
...
export const uploadImageToHosting=async ({hosting, url, projectId, label}:StoreHostedImageParams):Promise<HostedAsset | null> =>{
  ...
  try{
    ...
    const resolved = safeUrl.startsWith("data:")
      ? dataUrlToBlob(safeUrl)
      : label === "rendered"
        ? await imageUrlToPngBlob(safeUrl).then((blob) =>
            blob ? { blob, contentType: "image/png" } : null,
          )
        : await fetchBlobFromUrl(safeUrl);
